...
从旧版开放式 HTTP MCP 部署迁移到本版本时，请注意以下兼容性变化：

1. **Docker 部署强制认证**：容器启动时必须提供 `MCP_AUTH_TOKEN`。未设置时进程会拒绝启动，避免把未鉴权的 `/mcp` 暴露到网络。
2. **客户端必须带 Bearer**：所有访问 `/mcp` 的客户端需发送 `Authorization: Bearer <token>`。`/healthz` 仍保持无认证，供健康检查使用。
3. **默认只监听回环地址**：本地开发默认 `MCP_HTTP_HOST=127.0.0.1`。若改成非回环地址且未配置 token，服务器会拒绝启动。
4. **浏览器 Origin 白名单**：若从浏览器访问，必须配置 `MCP_ALLOWED_ORIGINS`（不支持 `*`）。原生 MCP 客户端不发送 `Origin` 时不受影响。
5. **建议迁移步骤**：
   - 生成长随机 token 并写入部署密钥/环境变量；
   - 更新 compose / k8s / systemd 环境变量；
   - 更新 MCP 客户端配置中的 headers；
   - 先用 `/healthz` 验证存活，再用带 token 的 `/mcp` 验证鉴权。

这些变更是为了堵住默认未鉴权的远程暴露面；本地 stdio 模式不受影响。
